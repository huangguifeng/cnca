...
class CncaSpider(scrapy.Spider):
    name = "cnca"
    ...
    def parse_captcha(self,response):
        ...
        captcha = self.tesseract(filePath)
        return captcha
    ...
    def insert_one_item(self,response,showtemp):
        ...
        item['cert_num'] = cert_info[0].xpath('.//td/text()')[1].extract()
        item['cert_status'] = cert_info[0].xpath('.//td/text()')[3].extract()

        item['pub_data'] = cert_info[1].xpath('.//td/text()')[1].extract()
        item['end_data'] = cert_info[1].xpath('.//td/text()')[3].extract()

        item['first_data'] = cert_info[2].xpath('.//td/text()')[1].extract()
        item['info_pub'] = cert_info[2].xpath('.//td/text()')[3].extract()

        item['cert_program'] = cert_info[3].xpath('.//td/text()')[1].extract()
        item['cert_according'] = cert_info[3].xpath('.//td/text()')[3].extract()

        item['supervise'] = cert_info[4].xpath('.//td/text()')[1].extract()
        item['echo_num'] = cert_info[4].xpath('.//td/text()')[3].extract()

        item['cert_work'] = cert_info[5].xpath('.//td/text()')[1].extract()
        item['paramcover_place'] = cert_info[6].xpath('.//td/text()')[1].extract()
        try:
            item['cert_addr'] = cert_info[7].xpath('.//td/text()')[1].extract()
        except:
            item['cert_addr'] = ""
        item['cert_mark'] = cert_info[8].xpath('.//td/text()')[1].extract()

        # 获证组织基本信息
        org_info = response.xpath('//table[@id="org_info"]//tr')
        item['org_name'] = org_info[0].xpath('.//td/text()')[1].extract()
        item['org_code'] = org_info[0].xpath('.//td/text()')[3].extract()
        item['org_people'] = org_info[1].xpath('.//td/text()')[3].extract()
        item['org_addr'] = org_info[2].xpath('.//td/text()')[1].extract()
        ...
        for node in cert_history:
            ...
            try:
                td = node.xpath('.//td')
                recp = re.compile(r'<.*?>')
                items['activity'] = recp.sub("", td[1].extract())
                items['describe'] = recp.sub("", td[2].extract())
                items['activeDate'] = recp.sub("", td[3].extract())
                items['checkGroup'] = recp.sub("", td[4].extract())
            except:
                pass
    ...
    def insert_two_item(self,response,showtemp):
        ...
        item['first_data'] = cert_info[2].xpath('.//td/text()')[1].extract()
        item['info_pub'] = cert_info[2].xpath('.//td/text()')[3].extract()
        item['according'] = cert_info[3].xpath('.//td/text()')[1].extract()
        item['product_type'] = cert_info[4].xpath('.//td/text()')[1].extract()
        item['master_product_name'] = cert_info[5].xpath('.//td/text()')[1].extract()
        item['product_name'] = cert_info[6].xpath('.//td/text()')[1].extract()
        item['model'] = cert_info[7].xpath('.//td/text()')[1].extract()

        app_info = response.xpath('//table[@id = "app_info"]//tr')

        item['consigner_name'] = app_info[0].xpath('//td/text()')[1].extract()  # 组织名称
        item['consigner_code'] = app_info[0].xpath('//td/text()')[3].extract()  # 组织机构代码
        item['consigner_area'] = app_info[1].xpath('//td/text()')[1].extract()  # 所在国别地区
        item['consigner_addr'] = app_info[1].xpath('//td/text()')[3].extract()  # 组织地址

        producer_info = response.xpath('//table[@id="manu_info"]//tr')
        item['producer_name'] = producer_info[0].xpath('//td/text()')[1].extract()  # 组织名称
        item['producer_code'] = producer_info[0].xpath('//td/text()')[3].extract()  # 组织机构代码
        item['producer_area'] = producer_info[1].xpath('//td/text()')[1].extract()  # 所在国别地区
        item['producer_addr'] = producer_info[1].xpath('//td/text()')[3].extract()  # 组织地址

        fac_info = response.xpath('//table[@id="fac_info"]//tr')
        item['manu_name'] = fac_info[0].xpath('//td/text()')[1].extract()  # 组织名称
        item['manu_code'] = fac_info[0].xpath('//td/text()')[3].extract()  # 组织机构代码
        item['manu_area'] = fac_info[1].xpath('//td/text()')[1].extract()  # 所在国别地区
        item['manu_addr'] = fac_info[1].xpath('//td/text()')[3].extract()  # 组织地址
        item['showtemp'] = showtemp
        return item
    ...
    def insert_three_item(self,response,showtemp):
        ...
        item['info_pub'] = cert_info[2].xpath('.//td/text()')[3].extract()
        item['cert_program'] = cert_info[3].xpath('.//td/text()')[1].extract()
        item['cert_mark'] = cert_info[4].xpath('.//td/text()')[1].extract()
        item['personnel_num'] = cert_info[4].xpath('.//td/text()')[3].extract()
        item['cert_work'] = cert_info[6].xpath('.//td//textarea/text()')[0].extract()
        item['according'] = cert_info[7].xpath('.//td/text()')[1].extract()
        try:
            item['cert_addr'] = cert_info[8].xpath('.//td/text()')[1].extract()
        except:
            item['cert_addr'] = ""
        # 组织机构
        ...
        for node in prod_info:
            items = {}
            items['product_type'] = node.xpath('.//td/text()')[0].extract()  # 产品类别
            items['product_name'] = node.xpath('.//td/text()')[1].extract()  # 产品名称
            items['product_addr'] = node.xpath('.//td/text()')[2].extract()  # 生产地址
            items['product_turnout'] = node.xpath('.//td/text()')[3].extract()  # 产量
            items['product_output'] = node.xpath('.//td/text()')[4].extract()  # 年产值
    ...
    def insert_four_item(self,response,showtemp):
        ...
        item['pub_data'] = cert_info[1].xpath('.//td/text()')[1].extract()
        item['end_data'] = cert_info[1].xpath('.//td/text()')[3].extract()
        item['info_pub'] = cert_info[2].xpath('.//td/text()')[1].extract()
        item['cert_program'] = cert_info[3].xpath('.//td/text()')[1].extract() # 认证项目
        item['cert_mark'] = cert_info[3].xpath('.//td/text()')[3].extract()  # 证书使用的认可标识
        item['cert_according'] = cert_info[4].xpath('.//td/text()')[1].extract()  # 认证依据
        item['product_name'] = cert_info[5].xpath('.//td/text()')[1].extract()  # 产品名称
        item['model'] = cert_info[6].xpath('.//td/text()')[1].extract()  # 规格型号

        app_info = response.xpath('//table[@id = "app_info"]//tr')

        item['consigner_name'] = app_info[0].xpath('//td/text()')[1].extract()  # 组织名称
        item['consigner_addr'] = app_info[0].xpath('//td/text()')[3].extract()  # 组织地址

        producer_info = response.xpath('//table[@id="manu_info"]//tr')
        item['producer_name'] = producer_info[0].xpath('//td/text()')[1].extract()  # 组织名称
        item['producer_addr'] = producer_info[0].xpath('//td/text()')[3].extract()  # 组织地址

        fac_info = response.xpath('//table[@id="fac_info"]//tr')
        item['manu_name'] = fac_info[0].xpath('//td/text()')[1].extract()  # 组织名称
        item['manu_addr'] = fac_info[0].xpath('//td/text()')[3].extract()  #
        item['showtemp'] = showtemp
        return item